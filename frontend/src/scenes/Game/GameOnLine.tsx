...
class Player {
  ...
  responsivePlayer(
    newWidth: number,
    newFatorEscalaX: number,
    newFatorEscalaY: number
  ) {
...
class Ball {
  ...
  printBall(positionX: number, positionY: number) {
    this.p5.circle(
      positionX * fatorEscalaX,
      positionY * fatorEscalaY,
      this.diameterBall
    );
  }
}
...
//   socketGame.connect();
  ...
  class Score {
    ...
    show(newScoreP1: number, newScoreP2: number) {
      ...
      if (this.score === null) {
        ...
        this.score.style(
          "font-size",
          `${(windowHeight < windowWidth ? windowHeight : windowWidth) * 0.1}px`
        );
        const size = Object.entries(this.score.size()).reduce<
        ...
        this.score.position(
          windowWidth / 2 - size["width"] / 2,
          windowHeight * 0.2 + parentBorderWidth
        );
      }
    }
    destroy() {
      if (this.score !== null) this.score = null;
    }
  }
    ...
    createButton() {
      if (this.button === null) {
        ...
        this.button.style(
          "font-size",
          `${
            (windowHeight < windowWidth ? windowHeight : windowWidth) * 0.08
          }px`
        );
        const size = Object.entries(this.button.size()).reduce<
          Record<string, number>
        >((acc, item) => {
          acc[item[0]] = item[1];
          return acc;
        ...
        this.button.position(
          windowWidth / 2 - size["width"] / 2,
          windowHeight / 2 + size["height"] / 2 + parentBorderWidth
        );
      }
    }
    destroy() {
      if (this.button !== null) this.button = null;
    }
  }
  ...
  const setup = (p5: p5Types, canvasParentRef: Element) => {
    parentBorderWidth =
      (ref.current ? ref.current.clientHeight : canvasParentRef.clientHeight) -
      canvasParentRef.clientHeight;
    windowHeight = ref.current
      ? ref.current.clientHeight - parentBorderWidth
      : canvasParentRef.clientHeight;
    windowWidth = ref.current
      ? ref.current.clientWidth - parentBorderWidth
  ...
  const windowResized = (p5: p5Types) => {
    parentBorderWidth =
      (ref.current ? ref.current.clientHeight : canvasParent.clientHeight) -
      canvasParent.clientHeight;
    const newWindowHeight = ref.current
      ? ref.current.clientHeight - parentBorderWidth
      : canvasParent.clientHeight;
    const newWindowWidth = ref.current
      ? ref.current.clientWidth - parentBorderWidth
